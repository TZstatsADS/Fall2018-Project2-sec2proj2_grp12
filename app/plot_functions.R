...
plot_delay_time = function(filtered_data,origin,destination) {
	...
	title = "Average Flight Delay Time in Minutes"
...
plot_delay_time_distribution = function(filtered_data,origin,destination) {
	...
	title = "Delay Time Distribution in Minutes"
...
plot_delay_ranking = function(filtered_data, origin, destination, month) {
  
  title = "# of Delay Flights by Carrier"
  ...
  p = ggplot(data = plot_data, aes(x = carrier, y=num_delays)) +
    geom_bar(stat='identity') +
    labs(title = title,
         x = 'Carrier',
